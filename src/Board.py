...
class Board:
    ...
    def table_as_tuple(self):
        return tuple(map(tuple, self.table))
    ...
    def bfs2(self):
        ...
        for move,letter in zip(avail_moves.values(), avail_moves.keys()):
            original_position = Index(self.empty_field_index.x, self.empty_field_index.y)
            self.move(move)
            que.append((self.table_as_tuple(), letter))
        ...
        while que:
            ...
            if self.is_solved():
                ...
                #     print(que.popleft())
                return og_tab[1]
            pos_moves = self.get_available_moves()
            if len(pos_moves.values()) > 0:
                for pos_move,letter in zip(pos_moves.values(),pos_moves.keys()):
                    if pos_move != original_position:
